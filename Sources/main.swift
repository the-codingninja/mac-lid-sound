...
class HingeDaemon {
    ...
    let velocityThreshold: Double = 8.0           // °/s to trigger sound (raised from 3)
    let stopThreshold: Double = 3.0               // °/s to stop sound
    ...
    let deadZone: Int = 3                         // minimum ° change from rest to trigger
...
// Resolve sounds directory relative to executable
...
if CommandLine.arguments.count > 1 {
    ...
    // Try relative to executable, then fall back to common locations
    let candidates = [
